...
1. **Create via CLI** using `--json`. The `content` field holds the full spec body as markdown. Use `\n` for newlines inside JSON strings.
...
2. `AskUserQuestion`: "How does this spec look?"
   ...
   - **Edit** → ask what to change, then update via `--json` and re-preview:

```bash
npx superintent spec update <id> --json '{"title": "{new title}", "content": "{updated content}"}'
```
