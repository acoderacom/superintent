...
- **All stale** → knowledge is untrustworthy, auto-lower confidence: `npx superintent knowledge update <id> --confidence <current - 0.15>` and tell the user
...
3. **`contentHash` is auto-generated by the CLI** from the `file:line` path — only `path` is required in citations. Cite **anchor lines** (function signatures, type definitions, route declarations) — if the anchor changes, the knowledge is stale. No need to cite every line in a body
