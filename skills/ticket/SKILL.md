...
2. `AskUserQuestion`: "How does this ticket look?"
   - **Looks good** → proceed
   - **Request changes** → ask what to change, then update via `--json` and re-preview:

```bash
npx superintent ticket update <id> --json '{"title": "{new title}", "context": "{updated context}", "plan": {...}}'
```
...
1. **Create via CLI** using `--json`. The `content` field holds the knowledge body. Use `\n` for newlines inside JSON strings
...
- **Update stale citations** — if cited files were modified during this ticket, update the knowledge entry with fresh citations (pass `path` only, CLI auto-generates `fileHash`):
  ```bash
  npx superintent knowledge update <id> --json '{"citations": [{"path": "file.ts:1"}, ...]}'
  ```
